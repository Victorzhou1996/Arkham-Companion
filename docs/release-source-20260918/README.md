...
- 这两份群友补丁不包含新图片。现有素材沿用：`img/arkham/customizations/09100.jpg` 为英文，`img/arkham/zh/customizations/09100.jpg` 已有中文“简易陷阱”升级说明。中文模式应使用后者，不应把英文目录的原图误认为唯一素材。
...
- Kaho 和 Online 已增量部署并保留旧入口备份，后端进程没有重启。Online 发现多处公共图片目录为 root:root、0700，使升级说明、封印等素材回退到 CDN；修复方式是备份目录权限记录，再对受影响的公共静态图片目录改为 0755，图片内容未变。Linux 分发时公共素材目录应可由服务用户遍历，图片应可读，不要用 root 专用权限封住这些目录。
