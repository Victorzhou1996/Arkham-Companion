...
: "${ARKHAM_SERVER_DATA:?Launch this server package through server.sh}"
PORT_CONFIG_FILE="$ARKHAM_SERVER_DATA/run/ports.env"
LAN_INFO_FILE="$ARKHAM_SERVER_DATA/run/lan.env"
DATA_DIR="$ARKHAM_SERVER_DATA/run"
NGINX_PID="$DATA_DIR/nginx.pid"
NGINX_LOG_DIR="$DATA_DIR"
API_PID_FILE="$DATA_DIR/arkham-api.pid"
PG_LOG="$DATA_DIR/pg.log"
RUNTIME_INFO_FILE="$DATA_DIR/local-runtime.json"
...
case "$(uname -s)" in
    ...
    Linux)  PGDATA_OS="$ARKHAM_SERVER_DATA/pgdata" ;;
esac
...
BACKUP_DIR="$ARKHAM_SERVER_DATA/backups"
...
refresh_pg_socket_dir() {
    PG_SOCKET_DIR="$ARKHAM_SERVER_DATA/run"
...
pg_ready(){ { "$(pg_bin)/pg_isready" -h 127.0.0.1 -p "$PG_PORT" -U "$PG_USER" -d postgres -q; } 2>/dev/null; }
...
restore_database_from_dump() {
    ...
    ensure_dir "$PG_DATA"
    if ! "$(pg_bin)/initdb" -D "$PG_DATA" -U "$PG_USER" --no-locale -E UTF8 --pwfile="$ARKHAM_PG_PASSWORD_FILE" --auth-host=scram-sha-256 \
...
start_postgres_for_admin() {
    ...
    if ! database_exists; then
        ...
        psql_cmd -d "$PG_DB" -v ON_ERROR_STOP=1 -f "$SCRIPT_DIR/data/setup.sql" >> "$DATA_DIR/psql.log" 2>&1 \
...
start_api_for_admin() {
    export DATABASE_URL="postgres://${PG_USER}:${PGPASSWORD}@127.0.0.1:${PG_PORT}/${PG_DB}"
...
generate_nginx_conf() {
    local conf="$DATA_DIR/nginx.conf"
    local frontend_root="$SCRIPT_DIR/frontend/dist"
    local pkg_root="$SCRIPT_DIR/.."
    local dns_resolvers
...
repair_running_frontend_assets() {
    ...
    if ! "$SCRIPT_DIR/bin/nginx" -e "$NGINX_LOG_DIR/error.log" \
        -c "$DATA_DIR/nginx.conf" -t >/dev/null 2>&1; then
        warn "The repaired nginx configuration did not pass validation"
        return 1
    fi
    if ! "$SCRIPT_DIR/bin/nginx" -e "$NGINX_LOG_DIR/error.log" \
        -c "$DATA_DIR/nginx.conf" -s reload >/dev/null 2>&1; then
...
do_stop() {
    ...
    if is_nginx_running; then
        ...
        "$SCRIPT_DIR/bin/nginx" -e "$NGINX_LOG_DIR/error.log" \
            -c "$DATA_DIR/nginx.conf" -s stop 2>/dev/null || true
...
do_repair_frontend() {
    ...
    if is_nginx_running; then
        ...
        "$SCRIPT_DIR/bin/nginx" -e "$NGINX_LOG_DIR/error.log" -c "$DATA_DIR/nginx.conf" -t >/dev/null 2>&1 \
...
init_database() {
    ...
    rm -f "$PG_LOG" "$DATA_DIR/psql.log" "$DATA_DIR/initdb.log" "$PG_RESTORE_LOG" 2>/dev/null || true
    ensure_dir "$PG_DATA"
    "$(pg_bin)/initdb" -D "$PG_DATA" -U "$PG_USER" --no-locale -E UTF8 --pwfile="$ARKHAM_PG_PASSWORD_FILE" --auth-host=scram-sha-256 \
...
do_start() {
    ...
    if [ "$is_fresh" = "1" ]; then
        ...
        psql_cmd -d "$PG_DB" -v ON_ERROR_STOP=1 -f "$SCRIPT_DIR/data/setup.sql" \
            >> "$DATA_DIR/psql.log" 2>&1 \
    ...
    sync 2>/dev/null || true
    if ! "$SCRIPT_DIR/bin/nginx" -e "$NGINX_LOG_DIR/error.log" -c "$DATA_DIR/nginx.conf" 2>&1; then
...
run_foreground() {
    if [ "${ARKHAM_HEADLESS:-0}" = 1 ]; then
        trap 'trap - EXIT; do_stop; exit 0' HUP INT TERM
        trap 'do_stop >/dev/null 2>&1 || true' EXIT
        info "Headless server ready; managed by the calling supervisor."
        local stats_refreshed_at
        stats_refreshed_at="$(date +%s)"
        while sleep 5; do
            if ! is_pg_running || ! is_api_running || ! is_nginx_running || ! local_layout_owned; then
                warn "A server process stopped; terminating for supervisor recovery."
                exit 1
            fi
