...
`start.bat` only exits with `0` (services started successfully) or `1` (WSL setup or service startup failed). On failure, scroll up in the cmd window to see which stage failed — the underlying `start.sh` error (1xxx/2xxx/3xxx) is printed before `start.bat` returns.
