...
function run(url,saved=null,blocked=false){
 ...
 vm.runInNewContext(script,{window,document,localStorage:storage,URL});return{result,window,document}
}
test('legacy switch preserves current game hash and query without touching saves',()=>{
 ...
 window.arkhamSwitchUi('legacy');assert.equal(result.navigated,'https://example.test/legacy-ui-20260921.1/?x=1&ui=legacy#/games/fixture');assert.equal(result.saved,'legacy')
})
test('explicit current recovery beats stale legacy preference and blocked storage',()=>{
 ...
 const blocked=run('https://example.test/legacy-ui-20260921.1/?ui=legacy#/games/g',null,true)
...
test('ordinary root entry restores selected legacy UI; only legacy gets floating return',()=>{
 assert.match(run('https://example.test/#/games/g','legacy').result.navigated,/legacy-ui-20260921.1/)
 assert.equal(run('https://example.test/').result.buttons.length,0)
 assert.equal(run('https://example.test/legacy-ui-20260921.1/','legacy').result.buttons.length,1)
})
...
test('shared editor keeps classic preference and does not redirect in a loop',()=>{
 ...
 const redirected=run('https://example.test/legacy-ui-20260921.1/?ui=legacy#/card-builder/marketplace','legacy')
