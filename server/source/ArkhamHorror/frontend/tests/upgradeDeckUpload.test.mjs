...
test('exported local deck JSON is normalized and submitted', async () => {
  const { loadUpgradeDeckFromJsonText } = await importTsModule(modulePath)
  const deck = {
    id: 'local-deck-id',
    name: 'Local campaign deck',
    investigatorName: 'Rex Murphy',
    url: null,
    list: {
  ...
  const result = loadUpgradeDeckFromJsonText(JSON.stringify(deck), {
    setModel: (value) => calls.push(['model', value]),
    setDeckList: (value) => calls.push(['deckList', value]),
    setDeckUrl: (value) => calls.push(['deckUrl', value]),
    setDeck: (value) => calls.push(['deck', value]),
  ...
  assert.deepEqual(result, { ok: true })
  assert.equal(calls[1][1].investigator_code, deck.list.investigator_code)
  assert.deepEqual(calls[1][1].slots, deck.list.slots)
  assert.deepEqual(calls[1][1].sideSlots, deck.list.sideSlots)
  assert.equal(calls[2][1], null)
  assert.equal(calls[4][1], deck.list.investigator_code)
})
