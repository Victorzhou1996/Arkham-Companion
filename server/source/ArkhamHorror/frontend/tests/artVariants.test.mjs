...
test('the player preference persists and resolves both cardImage and raw Art paths', async t => {
  ...
  const imagePath = url => new URL(url, 'http://localhost').pathname
  assert.deepEqual(settings.useVariants, [])
  assert.match(imagePath(cardImage('01149')), /cards\/01149\.avif$/)
  settings.setUseVariants(['unknown', 'revised', 'revised'])
  assert.equal(storage.get('arkhamUseVariants'), '["unknown","revised"]')
  assert.match(imagePath(cardImage('c01149', 'b')), /cards\/01649b\.avif$/)
  assert.match(imagePath(imgsrc('cards/01145b.avif')), /cards\/01645b\.avif$/)
  assert.match(imagePath(cardImage('01115')), /cards\/01115\.avif$/)
  setActivePinia(createPinia())
  assert.deepEqual(useSettings().useVariants, ['unknown', 'revised'])
  useSettings().setUseVariants([])
  assert.match(imagePath(imgsrc('cards/01145b.avif')), /cards\/01145b\.avif$/)
