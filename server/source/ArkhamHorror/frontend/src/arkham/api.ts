...
export const fetchArchiveStatus = async (gameId: string): Promise<boolean> => {
  const { data } = await api.get(`arkham/games/${gameId}/archive-status`)
  return data.archived === true
}

export const fetchFullGameExport = async (gameId: string): Promise<Blob> => {
  const { data } = await api.get(`arkham/games/${gameId}/full-export`, { responseType: 'blob' })
  return data
}
...
export const fetchBugReports = async (): Promise<BugReport[]> => {
  const { data } = await api.get('arkham/bugs')
  return data
}

export const loginBugAdmin = async (password: string): Promise<boolean> => {
  const { data } = await api.post('arkham/bugs/admin-login', { password })
  return data.ok === true
}
...
export const updateBugReport = async (
  ...
  const { data } = await api.put(`arkham/bugs/${bugId}`, { password, title, description })
  return data
}

export const deleteBugReport = async (bugId: string, password: string): Promise<void> => {
  await api.delete(`arkham/bugs/${bugId}`, { data: { password } })
}

export const exportBugReports = async (password: string): Promise<Blob> => {
  const { data } = await api.post('arkham/bugs/export', { password }, { responseType: 'blob' })
  return data
}
