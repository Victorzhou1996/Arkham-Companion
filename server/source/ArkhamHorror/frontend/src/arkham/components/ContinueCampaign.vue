...
const scenario = computed(() => {
  if (props.step.tag === 'ScenarioStep') {
    return props.step.contents.replace(/^c/, '')
  }
  if (props.step.tag === 'ScenarioStepWithOptions') {
    return props.step.contents[0].replace(/^c/, '')
  }
...
function requestCampaignStep(step: CampaignStep) {
  if (!canAnswer.value || hasSent.value) return
...
const canManageRoster = computed(() => !!props.campaign && canAnswer.value)
    ...
    <div v-else class="next-scenario">
      <div class="next-scenario-info">
        ...
        <div v-if="canAnswer" class="actions">
          ...
          <button v-if="canUpgrade" @click="upgradeDecks" :disabled="hasSent">{{t('upgradeDecks')}}</button>
          <button v-if="canChooseSideStory && standalones.length > 0" @click="addSideStory = true" :disabled="hasSent">+ {{t('addSideScenario')}}</button>
          <SideStoryOption
