...
async function pollChooseDecksState() {
  ...
  if (disposed || chooseDecksPolling || game.value?.gameState.tag !== 'IsChooseDecks') return
  chooseDecksPolling = true
  let delay = chooseDecksInterval()
  try {
    const step = await fetchGameStep(props.gameId)
    if (disposed || game.value?.gameState.tag !== 'IsChooseDecks') return
    if (step !== chooseDecksStep) {
      const latest = await fetchGame(props.gameId, props.spectate)
      if (disposed || game.value?.gameState.tag !== 'IsChooseDecks') return
      chooseDecksStep = step
      applyGameUpdate(latest.game, uiLock.value)
      updateGameLog(latest.game.log)
    ...
    chooseDecksPolling = false
    if (!disposed && game.value?.gameState.tag === 'IsChooseDecks') {
      chooseDecksPoll = setTimeout(pollChooseDecksState, delay)
    }
  }
}
...
watch(
  ...
  (tag) => {
    if (!disposed && tag === 'IsChooseDecks' && chooseDecksPoll === null && !chooseDecksPolling) {
...
watch(gameOver, async (ended) => {
  ...
  try {
    archived.value = await fetchArchiveStatus(props.gameId)
  } catch {
    archived.value = false
  } finally {
    archiveChecking.value = false
...
async function exportAndArchive() {
  ...
  try {
    const blob = await fetchFullGameExport(props.gameId)
    const url = URL.createObjectURL(blob)
    const link = document.createElement('a')
    link.href = url
    link.download = `arkham-full-export-${props.gameId}.json`
    link.click()
    window.setTimeout(() => URL.revokeObjectURL(url), 1000)
    await archiveGame(props.gameId)
    archived.value = true
  } catch (error: any) {
    window.alert(error?.response?.data?.message ?? t('archiveFailed'))
  } finally {
...
function applyGameUpdate(updatedGame: Arkham.Game, locked: boolean) {
  ...
  if (previousGame && entitiesMoved(previousGame, nextGame) && transitionDocument.startViewTransition) {
    ...
    // A skipped animation must not turn a successful game update into an unhandled rejection.
    ...
    .catch(async (err) => {
      ...
      await fetchGame(props.gameId, props.spectate)
        .then(({ game: refetched }) => {
          ...
          if (!uiLock.value) continueSkipAll()
        })
        .catch(() => {
          socketError.value = true
        })
...
async function runUndo(call: (gameId: string) => Promise<void>) {
  if (!canUseUndo.value || undoLock.value) return
  undoLock.value = true
  processing.value = true
...
async function fileBug() {
  ...
  Api.fileBug(props.gameId, {
    ...
    .then(() => {
      alert(t('gameBar.bugSubmittingDone'))
      bugTitle.value = ''
...
<template>
  ...
  <div id="game" ref="mobileGameRoot" v-else-if="ready && game && playerId" :inert="touchEnabled && !!mobilePreview" :class="{ 'tabletop-game': isActualScenarioView && !mobileEnabled, 'edge-tabletop': isActualScenarioView && !mobileEnabled, 'mobile-game': mobileEnabled, 'touch-game': touchEnabled, 'tablet-game': isActualScenarioView && tabletEnabled, 'mobile-tools-open': mobileTools, 'mobile-top-collapsed': mobileEnabled && mobileTopCollapsed, 'mobile-bottom-collapsed': mobileEnabled && mobileBottomCollapsed }" :data-mobile-zone="mobileEnabled ? mobileZone : undefined" :style="{ '--epic-bar-height': epicBarHeight + 'px', '--tabletop-tools-height': tabletopToolsHeight + 'px', ...(touchEnabled ? mobileViewport : {}) }">
    ...
    <Draggable v-if="filingBug">
      ...
      <form @submit.prevent="fileBug" class="bug-form box">
        <div class="bug-form-copy">
          <p>{{ $t('gameBar.fileBugPart1') }}</p>
          <p class="info">{{ $t('gameBar.fileBugPart2') }}</p>
          <p class="warning">{{ $t('gameBar.fileBugPart3') }}</p>
        </div>
        <label>
          <span>{{ $t('gameBar.bugTitleholder') }}</span>
          <input
            required
            type="text"
            v-model="bugTitle"
        ...
        <label>
          <span>{{ $t('gameBar.bugDescriptionholder') }}</span>
          <textarea
            required
        ...
        <div class="buttons">
          ...
          <button type="button" @click="filingBug = false">{{ $t('cancel') }}</button>
        </div>
      </form>
    </Draggable>
    ...
    <template v-else>
      ...
      <div v-else class="game-main">
        ...
        <div class="game-over" v-if="gameOver">
          ...
          <button
            v-if="onlineMode && !archived && !spectate"
            class="replay-button archive-button"
            :disabled="archiving || archiveChecking"
            @click="exportAndArchive"
          >
            {{ archiving ? $t('archivingGame') : $t('exportAndArchive') }}
          </button>
          <p v-if="archived" class="archive-status">{{ $t('gameArchived') }}</p>
