...
export function imgsrc(src: string) {
  ...
  if (isLocalized(src)) {
    ...
    if (exists && helper.root && helper.loaded.value) {
      ...
      if (canFetch) return companionCardSource(path, i18nFullPath, helper.root, store.companionAssetHost, language)
    }
  }
  
  return companionCardSource(path, fullPath, '', store.companionAssetHost, language)
}

function companionCardSource(path: string, fallback: string, languageRoot: string, companionHost: string, language: string) {
  if (!companionHost || language !== 'zh' || !/^cards\/[^/]+\.avif$/i.test(path)) return fallback

  const localPath = languageRoot ? `${languageRoot}/${path}` : path
  const absoluteFallback = typeof window === 'undefined'
    ? fallback
    : new URL(fallback, window.location.origin).href
  return `${companionHost}/img/arkham/${localPath}?fallback=${encodeURIComponent(absoluteFallback)}`
}
