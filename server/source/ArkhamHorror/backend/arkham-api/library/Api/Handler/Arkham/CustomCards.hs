...
storeArt userId contentType bytes = do
  ...
  let
    ...
    filename =
      tshow (hashlazy bytes :: Digest SHA256) <> "." <> extensionFor (Just contentType)

  getsApp (appCustomCardArtDir . appSettings) >>= \case
    -- Each deployment serves this persistent directory at /img/custom/.
    Just dir -> liftIO do
    ...
    Nothing -> invalidArgs ["Custom card image storage is not configured on this server"]
