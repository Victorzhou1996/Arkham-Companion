...
spec = describe "Underground River" do
  it "cannot be fully flooded by an effect that sets its flood level directly" . gameTest $ \self -> do
    ...
    river <- testLocationWithDef Locations.undergroundRiver id
    run $ RevealLocation (Just $ toId self) (toId river)
