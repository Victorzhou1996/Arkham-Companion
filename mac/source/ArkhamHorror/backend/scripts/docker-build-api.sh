...
jobs=${ARKHAM_BUILD_JOBS:-2}
ghc_build_options="-rtsopts -with-rtsopts=-V0 -j${jobs} +RTS -V0 -A128m -n2m -c -RTS"
stack build --jobs "$jobs" --no-terminal --system-ghc --ghc-options "$ghc_build_options"
stack --no-terminal --system-ghc --local-bin-path /opt/arkham/bin \
  install arkham-api --jobs "$jobs" --ghc-options "$ghc_build_options"
