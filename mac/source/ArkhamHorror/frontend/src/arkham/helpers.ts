...
const defaultHelper: ImageHelper = {
  root: '',
  digests: new Set(),
  loaded: ref(true),
}
const imgHelper: Map<string, ImageHelper> = new Map<string, ImageHelper>([
  ['it', { root: 'ita', digests: new Set(), loaded: ref(false) }],
  ['fr', { root: 'fr', digests: new Set(), loaded: ref(false) }],
  ['es', { root: 'es', digests: new Set(), loaded: ref(false) }],
  ['ko', { root: 'ko', digests: new Set(), loaded: ref(false) }],
  ['zh', { root: 'zh', digests: new Set(), loaded: ref(false) }],
])
...
const imageLanguage = (language: string) => language.toLowerCase().split('-')[0]

const imageAssetPath = (src: string, localizedRoot = '') => {
  const withoutQuery = src.split(/[?#]/, 1)[0]
  const marker = '/img/arkham/'
  const markerIndex = withoutQuery.indexOf(marker)
  const path = (
    markerIndex === -1 ? withoutQuery : withoutQuery.slice(markerIndex + marker.length)
  ).replace(/^\/+/, '')
  return localizedRoot && path.startsWith(`${localizedRoot}/`)
    ? path.slice(localizedRoot.length + 1)
    : path
}
...
const versionedUiAssets = new Set([
  'card.png',
  'resource.png',
  'clue.png',
  'health.png',
...
export function isLocalized(src: string) {
  ...
  const helper = imgHelper.get(language) || defaultHelper
  if (!helper.root || !helper.loaded.value) return false
  const path = imageAssetPath(src, helper.root)
  ...
  if (isLocalized(src)) {
    ...
    if (exists && helper.root && helper.loaded.value) {
      ...
      return cardImageSource(
        path,
        withUiAssetRevision(i18nFullPath, path),
