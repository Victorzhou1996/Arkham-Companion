...
test('circular actionable miniatures and compact buttons override broad styles without changing phones',()=>{
 ...
 assert.match(css,/\.player-buttons \.button-group \{ display: grid; grid-auto-flow: column/)
 assert.match(css,/#game.edge-tabletop \.player-container \.edge-personal-totals/)
 assert.match(read('arkham/components/Investigator.vue'),/:amount="game.totalClues" tooltip="Total Spendable Clues/)
})
