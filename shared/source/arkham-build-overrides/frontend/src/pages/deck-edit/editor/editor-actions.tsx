...
import { notifyDeckSaved } from "@/utils/deck-save-notification";
...
export function EditorActions(props: Props) {
  ...
  const { isSaving, onQuicksave, onSaveClose } = useSaveDeck(deck);
      ...
      <div className={css["actions"]}>
        ...
        >
          <Button
            ...
            onClick={onSaveClose}
            disabled={hasSyncConflict || readonly || isSaving}
            variant="primary"
          >
            <SaveIcon />
            {isSaving
              ? t("deck_edit.save_loading_short")
  ...
  const onSave = useCallback(
    async (stayOnPage?: boolean) => {
      ...
      try {
        ...
        notifyDeckSaved(String(id));
        toast.dismiss(toastId);
        toast.show({
          children: t("deck_edit.save_success"),
          variant: "success",
        });
        if (!stayOnPage) navigate(appPath(`/deck/view/${id}`));
      } catch (err) {
        toast.dismiss(toastId);
...
function useDiscardDeckEdits(deckId: ResolvedDeck["id"], hasEdits: boolean) {
  ...
  const onDiscard = useCallback(
    (stayOnPage?: boolean) => {
      ...
      if (confirmed) {
        discardEdits(deckId);
        if (!stayOnPage) navigate(appPath(`/deck/view/${deckId}`));
      }
    },
