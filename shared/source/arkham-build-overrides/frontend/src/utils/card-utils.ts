...
import { appPath, arkhamHorrorPath } from "./app-path";
...
export function cardBackTypeUrl(type: CardBackType) {
  if (isArkhamHorrorMode()) {
    const backs: Record<CardBackType, string> = {
      artifact: arkhamImageUrl("backs/back_artifact.jpg"),
      card: arkhamImageUrl("back.png"),
      cthulhu_deck: arkhamImageUrl("backs/back_cthulhu_deck.jpg"),
      encounter: arkhamImageUrl("encounter_back.jpg"),
      player: arkhamImageUrl("player_back.jpg"),
      the_longest_night: arkhamImageUrl("backs/back_the_longest_night.jpg"),
    };
    return backs[type];
  }
...
export function imageUrl(code: string) {
  if (isArkhamHorrorMode()) {
    return arkhamImageUrl(`zh/cards/${code}.avif`);
  }

  return `${import.meta.env.VITE_CARD_IMAGE_URL}/optimized/${code}.avif`;
}

function remoteCardImageUrl(
  code: string,
  directory: "optimized" | "thumbnails",
) {
  const host =
    import.meta.env.VITE_CARD_IMAGE_URL || "https://assets.arkham.build";
  return `${host.replace(/\/+$/, "")}/${directory}/${code}.avif`;
}

export function cardImageFallbackUrls(
  code: string,
  directory: "optimized" | "thumbnails",
  ...
  const urls = [
    ...
    remoteCardImageUrl(code, directory),
  ];
...
export function portraitUrl(code: string) {
  return arkhamImageUrl(`portraits/${code}.jpg`);
}
