...
export const createAdapter = {
  ...
  async persist(client: HttpClient, state: StoreState, deck: Deck) {
    if (isArkhamHorrorMode()) return createArkhamHorrorDeck(deck);
...
export const updateAdapter = {
  ...
  async persist(
    ...
    if (isArkhamHorrorMode()) {
      if (isArkhamHorrorDeck(deck)) return updateArkhamHorrorDeck(deck);
      if (deck.problem != null) return deck;
      return createArkhamHorrorDeck(deck);
    }
  ...
  transition(
    ...
    return set((prev) => {
      ...
      const idChanged = previousDeckId && previousDeckId !== deck.id;

      if (idChanged) {
        delete decks[previousDeckId];
        history[deck.id] = history[previousDeckId] ?? [];
        delete history[previousDeckId];

        if (deckFolders[previousDeckId]) {
          deckFolders[deck.id] = deckFolders[previousDeckId];
          delete deckFolders[previousDeckId];
        }

        if (undoHistory[previousDeckId]) {
          undoHistory[deck.id] = undoHistory[previousDeckId];
          delete undoHistory[previousDeckId];
        }
      }
...
export const deleteAdapter = {
  ...
  async persist(
    ...
    if (isArkhamHorrorMode()) {
      return isArkhamHorrorDeck(deck) ? deleteArkhamHorrorDeck(deck) : deck;
    }
...
export const upgradeAdapter = {
  ...
  async persist(
    ...
    if (isArkhamHorrorMode()) {
      if (!isArkhamHorrorDeck(deck)) return upgrade;
      return isActiveArkhamHorrorCampaignDeck(deck)
        ? updateArkhamHorrorDeck({
            ...upgrade,
            id: deck.id,
        ...
        : createArkhamHorrorDeck(upgrade);
    }
...
function updateDeckVersion(deck: Deck) {
  ...
  if (isArkhamHorrorDeck(deck)) return;
  deck.version = incrementVersion(deck.version);
}

function isActiveArkhamHorrorCampaignDeck(deck: Deck) {
  return (
    (decodeDeckMeta(deck) as Record<string, unknown>)[
      "arkham_horror_campaign_status"
    ] === "active"
  );
}
