...
describe("selectDecksDisplayList", () => {
  ...
  it("can hide campaign decks", async () => {
    const store = await getMockStore();
    ...
    store.setState({
      data: {
        ...
        decks: {
          ...
          completed: makeTestDeck({
            id: "completed",
            tags: "完成剧本的卡组",
          }),
          normal: makeTestDeck({ id: "normal", tags: "" }),
        },
        history: {
          active: [],
          completed: [],
      ...
      deckCollection: {
        ...store.getState().deckCollection,
        filters: {
          ...store.getState().deckCollection.filters,
          properties: {
            ...store.getState().deckCollection.filters.properties,
            hide_campaign: true,
          },
        },
      },
