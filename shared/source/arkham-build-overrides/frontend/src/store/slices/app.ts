...
import { getArkhamHorrorLocale } from "@/utils/i18n";
...
import {
  ...
  isArkhamHorrorDeck,
  isArkhamHorrorMode,
} from "../services/requests/arkham-horror";
...
export const createAppSlice: StateCreator<StoreState, [], [], AppSlice> = (
  ...
  async init(
    ...
    const arkhamHorrorLocale = isArkhamHorrorMode()
      ? getArkhamHorrorLocale()
      : undefined;
    const dataLocale =
      locale ??
      arkhamHorrorLocale ??
      ...
      get().settings.locale;
    const mergedOverrides: Partial<StoreState> = {
      ...overrides,
      settings: {
        ...get().settings,
        ...overrides?.settings,
    ...
    if (
      ...
      set((prev) => {
        const merged = mergeInitialState(prev, persistedState, mergedOverrides);
      ...
      await refreshArkhamHorrorDecks(set, get);
      return false;
    }

    time("query_data");
    const [metadataResponse, dataVersionResponse, cards] = await Promise.all([
      queryMetadata(dataLocale),
      queryDataVersion(dataLocale),
      queryCards(dataLocale),
    ]);
    timeEnd("query_data");
...
async function refreshArkhamHorrorDecks(
  set: Parameters<StateCreator<StoreState, [], [], AppSlice>>[0],
  get: Parameters<StateCreator<StoreState, [], [], AppSlice>>[1],
) {
  if (!isArkhamHorrorMode()) return;

  let remoteDecks: Deck[];
  try {
    remoteDecks = await fetchArkhamHorrorDecks();
  } catch (err) {
    console.error("[arkham-horror] failed to load decks", err);
    return;
  }
  ...
  set((state) => ({
    data: {
      ...state.data,
      deckFolders: Object.fromEntries(
        Object.entries(state.data.deckFolders ?? {}).filter(
          ([id]) => !isArkhamHorrorDeck(state.data.decks[id]),
        ),
      ),
      decks: {
        ...Object.fromEntries(
          Object.entries(state.data.decks).filter(
            ([, deck]) => !isArkhamHorrorDeck(deck),
          ),
        ),
        ...Object.fromEntries(remoteDecks.map((deck) => [deck.id, deck])),
      },
      history: {
        ...Object.fromEntries(
          Object.entries(state.data.history ?? {}).filter(
            ([id]) => !isArkhamHorrorDeck(state.data.decks[id]),
          ),
        ),
        ...Object.fromEntries(remoteDecks.map((deck) => [deck.id, []])),
      },
      undoHistory: Object.fromEntries(
        Object.entries(state.data.undoHistory ?? {}).filter(
          ([id]) => !isArkhamHorrorDeck(state.data.decks[id]),
        ),
      ),
    },
    deckEdits: Object.fromEntries(
      Object.entries(state.deckEdits ?? {}).filter(
        ([id]) => !isArkhamHorrorDeck(state.data.decks[id]),
      ),
    ),
    ui: {
      ...state.ui,
      fanMadeContentCache: buildCacheFromDecks([
        ...Object.values(state.data.decks).filter(
          (deck) => !isArkhamHorrorDeck(deck),
        ),
        ...remoteDecks,
      ]),
