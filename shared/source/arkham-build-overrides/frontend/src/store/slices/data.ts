...
import { parseDeckTransferData } from "../lib/deck-transfer";
...
export const createDataSlice: StateCreator<StoreState, [], [], DataSlice> = (
  ...
  async importDeck(client, input) {
    if (isArkhamHorrorMode()) {
      const deck = await importArkhamDbDeck(input);
      set((state) => ({
        data: {
          ...state.data,
          decks: { ...state.data.decks, [deck.id]: deck },
          history: { ...state.data.history, [deck.id]: [] },
        },
      }));
  ...
  async importFromFiles(files) {
    ...
    ).flatMap(parseDeckTransferData);

    if (!decks.length) {
      throw new Error("No valid deck data found in the selected JSON files.");
    }
    ...
    const imported = isArkhamHorrorMode()
      ? await Promise.all(formatted.map(createArkhamHorrorDeck))
      : formatted;
    ...
    set((state) => ({
      data: {
        ...
        decks: {
          ...state.data.decks,
          ...imported.reduce<Record<Id, Deck>>((acc, deck) => {
            acc[deck.id] = deck;
            return acc;
          }, {}),
        },
        history: {
          ...state.data.history,
          ...imported.reduce<Record<Id, string[]>>((acc, deck) => {
            acc[deck.id] = [];
            return acc;
          }, {}),
        },
