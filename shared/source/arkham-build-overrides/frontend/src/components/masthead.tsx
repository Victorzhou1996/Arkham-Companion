...
import {
  clearArkhamHorrorToken,
  fetchArkhamHorrorUser,
  isArkhamHorrorMode,
  type ArkhamHorrorUser,
} from "@/store/services/requests/arkham-horror";
import type { SyncStatus } from "@/store/slices/sync.types";
import { cx } from "@/utils/cx";
import {
  arkhamHorrorBasePath,
  arkhamHorrorHashPath,
} from "@/utils/app-path";
...
export function Masthead(props: Props) {
  ...
  if (arkhamHorrorMode) {
    const labels = arkhamHorrorNavLabels();
    const buildHref = `${arkhamHorrorBasePath()}/build/`;
    const decksHref = arkhamHorrorHashPath("decks");

    return (
      <header className={cx(className, css["arkham-masthead"])} id="masthead">
        <nav
          className={css["arkham-main-links"]}
          aria-label={t("masthead.navigation")}
        >
          <a className={css["arkham-home-link"]} href={arkhamHorrorHashPath()}>
            {labels.home}
          </a>
          <a className={css["arkham-nav-link"]} href={decksHref}>
            {labels.decks}
          </a>
          <a
            className={cx(css["arkham-nav-link"], css["active"])}
            href={buildHref}
          >
            Build
          </a>
          <a
            className={css["arkham-nav-link"]}
            href={arkhamHorrorHashPath("about?support")}
          >
            {labels.support}
          </a>
        </nav>
        <nav className={css["arkham-right"]}>
          {slotRight}
          <ArkhamHorrorAccountMenu />
        </nav>
      </header>
    );
  }
...
function arkhamHorrorNavLabels() {
  const lang =
    typeof window === "undefined" ? "zh" : localStorage.getItem("language");
...
function arkhamHorrorAccountLabels() {
  const lang =
    typeof window === "undefined" ? "zh" : localStorage.getItem("language");
  const isChinese = !lang || lang.toLowerCase().startsWith("zh");
...
function ArkhamHorrorAccountMenu() {
  const labels = arkhamHorrorAccountLabels();
  const [user, setUser] = useState<ArkhamHorrorUser | null>(null);
  ...
  const onLogout = useCallback(() => {
    clearArkhamHorrorToken();
    window.location.href = arkhamHorrorHashPath();
  }, []);
  ...
  return (
    <Popover>
      <PopoverTrigger asChild>
        <button
          className={css["arkham-user-button"]}
          disabled={loadingUser}
          type="button"
        >
          <span>{user?.username ?? labels.login}</span>
          <MenuIcon />
        </button>
      </PopoverTrigger>
      <PopoverContent>
        <DropdownMenu>
          {user && (
            <DropdownItem>
              <p className={css["arkham-logged-in"]}>{user.username}</p>
            </DropdownItem>
          )}
          <DropdownButton
            as="a"
            href={`${arkhamHorrorBasePath()}/build/settings`}
          >
            <SettingsIcon />
            {labels.settings}
          </DropdownButton>
...
function AccountMenu(props: {
  ...
  const arkhamHorrorMode = isArkhamHorrorMode();
  const accountSession = arkhamHorrorMode ? null : session;
          ...
          <DropdownItem>
            <p className={css["logged-in-as"]}>
              {t("auth.menu.logged_in_as", {
                name: accountSession.account.name,
              })}
            </p>
          </DropdownItem>
      ...
      {accountSession ? (
        <PopoverTrigger asChild>
          <Button
            ...
            size="none"
          >
            <Avatar account={accountSession.account}>
              <StatusBubble
