...
import { isArkhamHorrorMode } from "@/store/services/requests/arkham-horror";
...
const ARKHAM_BUILD_URL = "https://arkham.build/";
...
function SidebarActions(props: {
  ...
  const onExportToArkhamBuild = useCallback(() => {
    setActionsOpen(false);
    exportJson(deck.originalDeck);
    window.open(ARKHAM_BUILD_URL, "_blank", "noopener,noreferrer");
  }, [deck, exportJson]);
  ...
  useHotkey("cmd+i", onImport, { disabled: isLocal || arkhamHorrorMode });
  ...
  return (
    <>
      ...
      <div className={css["actions"]}>
        {origin === "local" ? (
          <>
            <Link href={`/deck/edit/${deck.id}`} asChild>
              ...
              <DropdownMenu>
                ...
                {origin === "local" && (
                  <>
                    ...
                    {arkhamHorrorMode && isLocalOnly && (
                      <DropdownButton
                        data-testid="view-export-arkham-build"
                        onClick={onExportToArkhamBuild}
                      >
                        <UploadIcon />
                        {t("deck_view.actions.export_arkham_build")}
                      </DropdownButton>
                    )}
                    <DropdownButton
      ...
      >
        {isSynced ? (
          <div className={css["share"]}>
            <ShareInfo deck={deck} path={deckDisplayPath(type, deck.id)} />
          ...
          <div className={css["share-empty"]}>
            ...
            <p>
              {t("deck_view.sharing.build_link")}
              <CopyToClipboard
                className={css["share-copy"]}
                text={deckDisplayUrl(type, deck.id)}
                tooltip={t("deck_view.sharing.copy_link")}
                variant="bare"
              />
            </p>
            {arkhamHorrorMode && (
              <>
                <nav className={css["share-actions"]}>
                  <Button
                    data-testid="share-export-arkham-build"
                    onClick={onExportToArkhamBuild}
                    size="sm"
                  >
                    <UploadIcon />
                ...
                <p>{t("deck_view.sharing.arkham_build_import_help")}</p>
              </>
            )}
            {!arkhamHorrorMode && availableUploadProviders.length > 0 && (
              <nav className={css["share-actions"]}>
                {availableUploadProviders.map((provider) => (
                  <Button
                    key={provider}
...
function deckDisplayPath(type: DeckDisplayType, id: Id) {
  const base = import.meta.env.BASE_URL.replace(/\/$/, "");
  return `${base}/${type}/view/${id}`;
}

function deckDisplayUrl(type: DeckDisplayType, id: Id) {
  return new URL(deckDisplayPath(type, id), window.location.origin).toString();
}
