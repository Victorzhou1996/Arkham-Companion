...
export function CardScanControlled(props: Props) {
  ...
  // Fan-made content uses card urls for sides, these take precedence outside the embedded app.
  const frontUrl = isArkhamHorrorMode()
    ? undefined
    : suffix === "b"
      ? card.back_image_url
      : card.image_url;
  const frontRemoteUrl = suffix === "b" ? card.back_image_url : card.image_url;
  const backUrl = isArkhamHorrorMode()
    ? undefined
    : backCard
      ? backCard.image_url
      : suffix === "b"
    ...
    >
      <div className={css["scan-front"]} data-testid="card-scan" ref={scanRef}>
        <CardScanInner
          ...
          fallbackUrls={cardImageFallbackUrls(
            imageCode,
            "optimized",
            frontRemoteUrl,
          )}
    ...
    >
      {shown && (
        <img
          ...
          onError={(event) =>
            advanceImageFallback(event.currentTarget, fallbackUrls)
          }
          src={url}
